...
def read(  # noqa: C901
    filename: Path | str, events_per_chunk: int, parser: str = "pandas", centrality_value_to_inject: float | None = None
) -> Generator[ak.Array, int | None, None]:
            ...
            if centrality_value_to_inject:
                ...
                if i == 0:
                    ...
                    if chunk_generator.model_parameters.model_name != "hybrid":
                        ...
                        raise RuntimeError(msg)
                header_level_info["centrality"] = np.full(
                    len(chunk_generator.headers), centrality_value_to_inject, dtype=np.float32
                )
...
def parse_to_parquet(
    ...
    for i, arrays in enumerate(
        read(
            ...
            parser=parser,
            centrality_value_to_inject=centrality_value_to_inject,
        )
    ):
